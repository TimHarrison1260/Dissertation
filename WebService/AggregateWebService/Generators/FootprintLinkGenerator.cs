...
{
    /// <summary>
    /// Class <c>FootprintLinkGenerator</c> has the responsibility
    /// to generate the Footprint Hypermedia Links for the aggregate 
    /// entry being returned through the web service.
    /// </summary>
    public class FootprintLinkGenerator : IFootprintLinkGenerator
    {
        /// <summary>
        ...
        public IEnumerable<Models.Link> GenerateCollectionLinks(Core.Model.Aggregate aggregate)
        {
            var links = new List<Link>()
            {
                new Link()
                {
                    Href = "/api/footprint/" + aggregate.Id,
                    Rel = "self",
                    Title = string.Empty,
                    Type = "method=\"GET\""
                },
                new Link()
                {
                    Href = "/api/windfarm/" + aggregate.Id + "/footprint/",
                    Rel = "windfarm footprint",
                    Title = string.Empty,
                    Type = "method=\"GET\""
                },
                new Link()
        ...
        /// <summary>
        /// Generates the Footprint links for an aggregate when a specific
