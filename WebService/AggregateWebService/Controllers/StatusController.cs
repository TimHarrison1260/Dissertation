...
{
    /// <summary>
    /// Supports api/status/5 or windfarm/5/status for HttpVerb Get only
    /// </summary>
    public class StatusController : ApiController
    {
        ...
        private readonly IStatusLinkGenerator _generator;     //  Module to create the Hypermedia Links
        ...
        public StatusController(IDataService dataService, IAggregateMapper aggregateMapper, IStatusLinkGenerator generator)
        {
            if (dataService == null)
        ...
        /// <summary>
        /// GET api/status
        /// : Get all windfarms that have a Status datatype segment
        /// </summary>
        /// <returns>A collection of windfarms</returns>
        //public IEnumerable<WindfarmInfo> Get()
        public HttpResponseMessage Get()
        {
            var aggregates = _service.GetAggregatesWithDataType(DataTypeEnum.Status).ToArray();
            if (!aggregates.Any())
        ...
        /// <summary>
        /// GET api/status/5
        /// : Get the windfarm with Id 5, and included only the Status datatype segment
        /// </summary>
        ...
        {
            var aggregate = _service.GetAggregateWithDataType(id, DataTypeEnum.Status);
            if (aggregate == null)
            {
            ...
            var uiAggregate = _mapper.MapAggregate(aggregate, DataTypeEnum.Status, _generator);
