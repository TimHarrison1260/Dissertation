...
namespace AggregateWebService.Controllers
{
    ...
    public class TurbineController : ApiController
    {
        ...
        private readonly ITurbineLinkGenerator _generator;     //  Module to create the Hypermedia Links
        ...
        /// <summary>
        /// GET api/footprint
        /// : Get all windfarms that have a Turbine datatype segment
        /// </summary>
        /// <returns>A collection of windfarms</returns>
        //public IEnumerable<WindfarmInfo> Get()
        public HttpResponseMessage Get()
        {
            var aggregates = _service.GetAggregatesWithDataType(DataTypeEnum.Turbine).ToArray();
            if (!aggregates.Any())
        ...
        {
            var aggregate = _service.GetAggregateWithDataType(id, DataTypeEnum.Turbine);
            if (aggregate == null)
            {
            ...
            var uiAggregate = _mapper.MapAggregate(aggregate, DataTypeEnum.Turbine, _generator);
